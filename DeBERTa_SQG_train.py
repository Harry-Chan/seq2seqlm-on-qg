...
def convert_data_to_features(args, datas, tokenizer):
    ...
    for index, ele in enumerate(tqdm(datas)):
        try:
            ...
            max_context_length = args.max_seq_length - len(answer_tokens) - len(question_tokens) - 4
...
def train(args, model, tokenizer):
    ...
    """ Load datas """
    train_dataset = read_data(args.train_file)
    ...
    # Init wandb 
    wandb.init(project="DeBERTa_SQuAD",
        name="debarta-base"+"_{0}_{1}_{2}_{3}".format(str(args.max_seq_length), str(args.doc_stride), str(args.max_answer_length), str(args.max_query_length))
        )
    wandb.watch(model)
