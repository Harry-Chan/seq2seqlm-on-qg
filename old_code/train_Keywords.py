...
def convert_data_to_features(args, datas, tokenizer):
    ...
    for index, ele in enumerate(tqdm(datas)):
        try:
            ...
            for i, keyword in enumerate(ele.keywords):
                keywords += ' ' + keyword

            keywords_tokens = tokenizer.tokenize(keywords)
            ...
            if len(keywords_tokens) > args.max_query_length:
                continue
                # question_tokens = question_tokens[0:args.max_query_length] 

            max_context_length = args.max_seq_length - len(answer_tokens) - len(keywords_tokens) - 4
            ...
            keywords_tokens += [sep_token]
            keyword_ids = tokenizer.convert_tokens_to_ids(keywords_tokens)
            ...
            for keyword_id in keyword_ids:               
                ...
                label_ids[label_indexs] = keyword_id
                ...
                if index < 20 :
                    ...
                    logger.info("output_ids: %s" % " ".join([str(x) for x in keyword_ids]))
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    """ Load datas """
    eval_dataset = read_data(args, args.predict_file)
