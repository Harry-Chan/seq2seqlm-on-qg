...
class ModelEvalMixin:
    def write_predict(self, decode_question, ref_question):
        ...
        with open(
            os.path.join(log_dir, "predict_for_nqg_scorer.txt"), "a", encoding="utf-8"
        ) as log_f:
    ...
    def evaluate_predict(self, dataset):
        ...
        if dataset == "squad":
            os.system(
                "python nqg/qgevalcap/eval.py --src nqg/data/processed/src-test.txt --tgt nqg/data/processed/tgt-test.txt --out %s >> %s"
